...
const PolicyForm = () => {
    ...
    if (isEditMode) {
      const fetchData = async () => {
        try {
          ...
          alert("Failed to load policy data for editing.");
        } finally {
          setIsLoading(false);
        }
      };
  ...
  const handleSubmit = async (e) => {
    ...
    // Also send the old file path if in edit mode so the backend can delete it
    if (isEditMode && formData.document) {
  ...
  return (
    ...
    >
      <Header title={isEditMode ? "Edit Policy" : "Add Policy"} onGoBack={handleGoBack} />
            ...
            <div>
                ...
                {isEditMode && !formData.document && existingDocument && (
                    <div className="mt-2 text-sm text-gray-600">
                        Current file: <a href={`http://localhost:8080${existingDocument}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">{existingDocument.split('/').pop()}</a>
                    </div>
                )}
            </div>
        </div>
