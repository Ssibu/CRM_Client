...
// Define the API endpoint for Schemes
...
const Scheme = () => {
  ...
  const handleToggleConfirm = async () => {
    if (modalState.itemToToggle) {
      const { id } = modalState.itemToToggle;
      try {
        await axios.patch(`${API_URL}/status/${id}`);
        fetchData(); // Refetch data to show the updated status
  ...
  // --- Define the table structure for Schemes ---
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const fileUrl = `http://localhost:7777${filename}`;
        ...
        const getIcon = () => {
          ...
          // ===== CORRECTED: EXCEL FILE CHECK ADDED =====
          if (['xls', 'xlsx'].includes(extension)) return <FaFileExcel className="text-green-700" size={22} />;
          
          // Add other file types like images if your uploader supports them
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex space-x-2">
          ...
          <button 
            onClick={() => navigate(`/admin/notifications/scheme/edit/${row.original.id}`)} 
