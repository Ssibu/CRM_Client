...
//       isSortable: true,
//     },
//     { header: "SubMenu (in English)", accessor: "en_title", isSearchable: true, isSortable: true },
//     { header: "SubMenu (in Odia)", accessor: "od_title", isSearchable: true, isSortable: true },
//     {
//         header: "Image",
//         accessor: "image_url",
//         cell: ({ row }) => (
...
//           message={`Are you sure you want to set "${modalState.itemToUpdate?.en_title}" to ${modalState.itemToUpdate?.status === 'Active' ? 'Inactive' : 'Active'}?`}
//           icon={AlertTriangle}
...
const SubMenu = () => {
  ...
  const handleStatusUpdateConfirm = async () => {
    ...
    try {
      const response = await axios.put(
        `${API_BASE_URL}/api/submenus/status/${item.id}`,
        { status: newStatus },
        { withCredentials: true }
      );
      const updatedItem = response.data;
      setPaginatedSubMenus((prevData) =>
        prevData.map((submenu) => (submenu.id === updatedItem.id ? updatedItem : submenu))
      );
      showModal("success", `SubMenu status set to "${newStatus}" successfully!`);
      refreshData();
    } catch (error) {
  ...
  const handleSaveOrder = async (newOrder) => {
    ...
    try {
      await axios.put(
        `${API_BASE_URL}/api/submenus/order`,
        { order: orderIds },
        { withCredentials: true }
      );
      setShowSortModal(false);
      ...
      { header: "SubMenu (in Odia)", accessor: "od_title", isSearchable: true, isSortable: true },
      {
        ...
        cell: ({ row }) => {
          const imgUrl = row.original.image_url
            ? `${API_BASE_URL}/uploads/submenus/${row.original.image_url.split("/").pop()}`
            : null;
          return imgUrl ? (
            <img
              src={imgUrl}
              ...
              onClick={() => setPreviewImage(imgUrl)} // Open image modal on click
            />
          ) : (
    ...
    <div className="min-h-[80vh] py-4 font-sans">
      ...
      {modalState.isOpen && (
        <DeleteConfirmationModal
          ...
          onConfirm={handleStatusUpdateConfirm}
          title="Confirm Status Change"
          message={`Are you sure you want to set "${modalState.itemToUpdate?.en_title}" to ${
            modalState.itemToUpdate?.status === "Active" ? "Inactive" : "Active"
          }?`}
          icon={AlertTriangle}
          confirmText={
            modalState.itemToUpdate?.status === "Active" ? "Set to Inactive" : "Set to Active"
          }
          confirmButtonVariant={
            modalState.itemToUpdate?.status === "Active" ? "danger" : "success"
          }
        />
      )}
