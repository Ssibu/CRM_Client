...
//         <img 
//           src={`${API_BASE_URL}/uploads/menus/${row.original.image_url}`} 
  ...
  const handleStatusUpdateConfirm = async () => {
    ...
    try {
      const response = await axios.put(
        `${API_BASE_URL}/api/menus/status/${item.id}`,
        { status: newStatus },
      ...
      setPaginatedMenus((prevData) =>
        prevData.map((menu) => (menu.id === updatedItem.id ? updatedItem : menu))
      );
      showModal("success", `Menu status set to "${newStatus}" successfully!`);
      refreshData();
    } catch (error) {
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const imgUrl = row.original.image_url
            ? `${API_BASE_URL}/uploads/menus/${row.original.image_url}`
            : null;
          return imgUrl ? (
            <img
              src={imgUrl}
