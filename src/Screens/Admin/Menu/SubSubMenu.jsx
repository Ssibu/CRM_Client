...
//     { header: "Sub-SubMenu (in English)", accessor: "en_title", isSearchable: true, isSortable: true },
//     { header: "Sub-SubMenu (in Odia)", accessor: "od_title", isSearchable: true, isSortable: true },
//     {
//         header: "Image",
//         accessor: "image_url",
//         cell: ({ row }) => (
...
const SubSubMenu = () => {
  ...
  const columns = useMemo(() => [
    ...
    { header: "Sub-SubMenu (in Odia)", accessor: "od_title", isSearchable: true, isSortable: true },
    {
      ...
      cell: ({ row }) => {
        const imgUrl = row.original.image_url 
          ? `${API_BASE_URL}/uploads/subsubmenus/${row.original.image_url.split('/').pop()}`
          : null;
        return imgUrl ? (
          <img 
            src={imgUrl} 
            ...
            onClick={() => setPreviewImage(imgUrl)}  // <-- open modal on click
          />
        ) : (
    ...
    },
    {
      ...
      isSortable: true,
      cell: ({ row }) => (
        <span className={`px-2 py-1 text-xs font-semibold rounded-full ${row.original.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
          {row.original.status}
        </span>
      ),
    },
