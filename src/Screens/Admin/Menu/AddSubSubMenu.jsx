...
const AddSubSubMenuForm = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      setIsLoading(true);
      try {
        // ✅ FIX #1: Use dedicated '/list' endpoints that return flat arrays
        ...
        if (isEditMode) {
          ...
          // ✅ FIX #2: Set the form data directly from the detailed fetch response
          setFormData({
            menu: data.SubMenu ? data.SubMenu.menuId : "", // Get parent menu ID directly
        ...
        setModalMessage("Could not load initial form data.");
        setModalOpen(true);
      } finally {
        setIsLoading(false);
      }
  ...
  // (The rest of your component logic remains the same as it is already correct)
  // ... handleInputChange, handleImageChange, handleRemoveImage, handleSubmit, etc.
