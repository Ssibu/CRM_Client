...
import ImageModal from "../../../Components/Admin/ImageModal/ImageModal"; // ✅ Import Image Modal
...
const HomepageBannerList = () => {
  ...
  const confirmStatusToggle = useCallback(async () => {
    ...
    try {
      ...
      showModal(
        "success",
        `Banner with SL.No ${selectedBannerSlNo} is now ${
          updatedBanner.is_active ? "Active" : "Inactive"
        }.`
      );
    } catch (error) {
      console.error(
        ...
        cell: ({ row }) => {
          ...
          return (
            <img
              ...
              className="h-12 w-24 rounded object-cover cursor-pointer hover:scale-105 transition"
              onClick={() => setPreviewImage(imgSrc)} // ✅ Open image modal
            />
          );
        },
      },
