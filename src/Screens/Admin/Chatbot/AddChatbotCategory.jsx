...
const AddChatbotCategory = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.en_title.trim()) {
      ...
      errors.en_title = "English category name must be at least 2 characters";
    }
    
    if (!formData.od_title.trim()) {
      errors.od_title = "Odia category name is required";
    } else if (formData.od_title.trim().length < 2) {
      errors.od_title = "Odia category name must be at least 2 characters";
    }
  ...
  const checkForDuplicates = async () => {
    try {
      ...
      const enValue = formData.en_title.trim().toLowerCase();
      const odValue = formData.od_title.trim();

      const englishDuplicate = categories.find(
        (cat) =>
          cat.en_title && cat.en_title.trim().toLowerCase() === enValue &&
          (!isEditMode || cat.id !== parseInt(id))
      );

      const odiaDuplicate = categories.find(
        (cat) =>
          cat.od_title && cat.od_title.trim() === odValue &&
          (!isEditMode || cat.id !== parseInt(id))
      );
