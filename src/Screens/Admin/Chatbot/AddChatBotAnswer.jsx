...
//     else if (trimmedEn.length < 3) errors.en_answer = "English answer must be at least 3 characters";
//     if (!trimmedOd) errors.od_answer = "Odia answer is required";
//     else if (trimmedOd.length < 3) errors.od_answer = "Odia answer must be at least 2 characters";
...
//   onCancel={() => navigate("/admin/manage-chatbot/chatbot-answer")}
//   onReset={handleReset}
// />
...
const AddChatbotAnswer = () => {
  ...
  // NEW: State to store the original data for resetting in edit mode.
  ...
  // Fetch all categories (no changes here)
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await chatbotCategoryAPI.getAll(1, 1000, "");
  ...
  // Fetch questions when category changes (no changes here)
  useEffect(() => {
    const fetchQuestions = async () => {
      if (formData.category_id) {
        setLoading(true);
    ...
    if (isEditMode) {
      const fetchAnswerData = async () => {
        ...
        try {
          ...
          // CHANGED: Create a structured data object from the fetched answer.
          const fetchedData = {
            category_id: answer.category_id || "",
            question_id: answer.question_id || "",
            en_answer: answer.en_answer || "",
          ...
          // CHANGED: Set both the form data (for editing) and the original data (for resetting).
  ...
  // Check for Duplicates (no changes here)
  const checkForDuplicates = async () => {
    try {
      const response = await chatbotAnswerAPI.getAll(1, 1000, "");
      const answers = response.data.answers || [];
  ...
  const handleReset = () => {
    ...
    // In 'edit' mode, originalData is the fetched data.
