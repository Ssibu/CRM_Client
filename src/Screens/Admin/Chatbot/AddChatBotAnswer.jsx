...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    if (isEditMode) {
      response = await chatbotAnswerAPI.update(id, submitData);
      setModal({
        open: true,
        variant: "success",
        message: response?.data?.message || "Answer updated successfully!",
      });
    } else {
      response = await chatbotAnswerAPI.create(submitData);
      setModal({
        open: true,
        variant: "success",
        message: response?.data?.message || "Answer created successfully!",
      });
    }
    ...
    if (response?.status === 400) {
      ...
      // Just show the field-level errors — no 'Validation failed'
      let message = "";
      ...
      if (errors) {
        ...
        // Fallback to the backend message if errors object is not present
