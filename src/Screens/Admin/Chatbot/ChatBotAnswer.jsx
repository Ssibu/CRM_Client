...
//         <td className="p-2">{item.category?.en_answer || "N/A"}</td>
//         <td className="p-2">{item.question?.en_answer || "N/A"}</td>
//         <td className="p-2">{truncateText(item.en_answer)}</td>
//         <td className="p-2 font-odia">{truncateText(item.od_answer)}</td>
  ...
  return (
    <>
      <tr className="border-b border-gray-300 hover:bg-gray-50 transition-all">
        ...
        <td className="p-2">{item.category?.en_title || "N/A"}</td>
        <td className="p-2">{item.question?.en_question || "N/A"}</td>
        <td className="p-2">{truncateText(item.en_answer)}</td>
        <td className="p-2 font-odia">{truncateText(item.od_answer)}</td>
        <td className="p-2 text-center">
...
const sortAnswers = (answers, sortConfig) => {
  ...
  sortedAnswers.sort((a, b) => {
    // Handle nested fields like category.en_answer
...
const filterAnswers = (answers, searchTerm) => {
  ...
  return answers.filter(item => 
    (item.category?.en_title && item.category.en_title.toLowerCase().includes(searchLower)) ||
    (item.question?.en_question && item.question.en_question.toLowerCase().includes(searchLower)) ||
    (item.en_answer && item.en_answer.toLowerCase().includes(searchLower)) ||
    (item.od_answer && item.od_answer.toLowerCase().includes(searchLower)) ||
  ...
  return (
    <div className="p-4 bg-white shadow rounded-xl overflow-x-auto">
      ...
      <table className="min-w-full border text-sm">
          ...
          <tr className="bg-gray-100 text-left">
            ...
            <th className="p-2">
              <div className="flex items-center">
                Category
                <SortArrow column="en_title" currentSort={sort} onSortChange={handleSortChange} />
              </div>
            </th>
            <th className="p-2">
              <div className="flex items-center">
                Question
                <SortArrow column="question.en_answer" currentSort={sort} onSortChange={handleSortChange} />
              </div>
            </th>
            <th className="p-2">
              <div className="flex items-center">
                Answer (English)
                <SortArrow column="en_answer" currentSort={sort} onSortChange={handleSortChange} />
              </div>
            </th>
            <th className="p-2">
              <div className="flex items-center">
                Answer (Odia)
                <SortArrow column="od_answer" currentSort={sort} onSortChange={handleSortChange} />
              </div>
            </th>
        ...
        <tbody>
          {loading ? (
            <tr><td colSpan="7" className="text-center p-4">
