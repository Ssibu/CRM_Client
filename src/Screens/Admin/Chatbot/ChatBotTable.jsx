...
const TableRow = ({ item, idx, currentPage, entriesPerPage, searchTerm, onStatusChange, showModal }) => {
  ...
  const handleStatusToggle = async () => {
    ...
    try {
      ...
      showModal("success", `Category "${item.en_title}" has been ${newStatus === "Active" ? "activated" : "deactivated"} successfully!`);
    } catch (error) {
  ...
  return (
    <>
      <tr className="border-b border-gray-300 hover:bg-gray-50 transition-all">
        ...
        <td className="p-2">{highlightMatch(item.en_title, searchTerm)}</td>
        <td className="p-2 font-odia">{highlightMatch(item.od_title, searchTerm)}</td>
        <td className="p-2">
          <span
      ...
      {isConfirmModalOpen && (
        <DeleteConfirmationModal
          ...
          message={
            item.status === "Active"
              ? `Are you sure you want to deactivate "${item.en_title}"? It will no longer be visible to users.`
              : `Are you sure you want to activate "${item.en_title}"? It will become visible to users.`
          }
...
const ChatBotTable = () => {
  ...
  const [sortField, setSortField] = useState("en_title"); // default sort field
        ...
        <thead>
          <tr className="bg-gray-100 text-left">
            ...
            <th
              ...
              onClick={() => handleSortToggle("en_title")}
            >
              Category (English) {renderSortIcon("en_title")}
            </th>
            <th
              className="p-2 cursor-pointer select-none"
              onClick={() => handleSortToggle("od_title")}
            >
              Category (Odia) {renderSortIcon("od_title")}
            </th>
