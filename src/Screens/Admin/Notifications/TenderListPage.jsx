...
const TenderListPage = () => {
  ...
  const tenderColumns = useMemo(() => [
    ...
    { header: "Date", accessor: "date", isSortable: true,  cell: ({ row }) => formatDate(row.original.date), },
    { header: "Expiry Date", accessor: "expiry_date", isSortable: true,  cell: ({ row }) => formatDate(row.original.expiry_date), },
    {
      header: "NIT Document",
      cell: ({ row }) => (
