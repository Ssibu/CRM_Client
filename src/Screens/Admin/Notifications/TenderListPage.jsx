...
const TenderListPage = () => {
  ...
  const {
    ...
    setData,
    tableState,
  } = useServerSideTable(`${import.meta.env.VITE_API_BASE_URL}/tenders`);
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-4">
            {row.original.nit_doc ? (
              <a
                href={`${import.meta.env.VITE_API_BASE_URL}/uploads/tenders/${
                  row.original.nit_doc
                }`}
                target="_blank"
                rel="noopener noreferrer"
                title="View NIT Document"
              >
        ...
        cell: ({ row }) => (
          <div className="flex items-center space-x-3">
            ...
            <button
              onClick={() =>
                navigate(
                  `/admin/notifications/tenders/${row.original.id}/corrigendums`
                )
              }
              className="text-purple-500"
              title="Manage Corrigendums"
            >
              <FaListAlt />
            </button>
          </div>
      ...
      {isModalOpen && selectedTender && (
        <DeleteConfirmationModal
          ...
          message={`Are you sure you want to ${
            selectedTender.is_active ? "deactivate" : "activate"
          } "${selectedTender.en_title}"?`}
