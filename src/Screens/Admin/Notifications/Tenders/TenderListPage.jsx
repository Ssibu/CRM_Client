  ...
  const handleToggleStatus = async () => {
    ...
    try {
      const res = await axios.patch(
        `${process.env.REACT_APP_API_URL}/tenders/${selectedTender.id}/status`, 
        {}, 
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-4">
          {row.original.nit_doc && (
            <a href={`${process.env.REACT_APP_API_URL}/uploads/tenders/${row.original.nit_doc}`} target="_blank" rel="noopener noreferrer" title="View NIT Document">
    ...
    },
     {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-4">
          ...
          {row.original.doc && (
             <a href={`${process.env.REACT_APP_API_URL}/uploads/tenders/${row.original.doc}`} target="_blank" rel="noopener noreferrer" title="View Other Document">
