...
const CorrigendumList = ({ corrigendums, onEdit, onDataChange }) => {
    ...
    const handleToggleStatus = async () => {
        ...
        try {
            await axios.patch(
                `${process.env.REACT_APP_API_URL}/corrigendums/${selectedCorrigendum.id}/status`,
                {},
                { withCredentials: true }
            );
            showModal("success", `Corrigendum status has been updated.`);
            onDataChange(); // Notify the parent page to re-fetch all data
        } catch (error) {
    ...
    const columns = useMemo(() => [
        {
            header: 'SL.No',
            // Our MenuTable provides these values even in client-side mode
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <a 
                  href={`${process.env.REACT_APP_API_URL}/uploads/tenders/corrigendums/${row.original.cor_document}`} 
                  target="_blank" 
