...
const AdvertisementFormPage = () => {
    ...
    const fetchAdData = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/advertisements/${id}`,
        ...
        const fetchedFileNames = {
          en_adphoto: en_adphoto || "",
          od_adphoto: od_adphoto || "",
        };
        ...
        showModal(
          "error",
          err.response?.data?.message || "Failed to fetch advertisement data."
        );
  ...
  const handleSubmit = async (e) => {
    ...
    if (filesToRemove.en_adphoto)
      submissionData.append("remove_en_adphoto", "true");
    if (filesToRemove.od_adphoto)
      submissionData.append("remove_od_adphoto", "true");
    ...
    try {
      ...
      showModal(
        "success",
        `Advertisement ${isEditMode ? "updated" : "added"} successfully!`
      );
      navigate("/admin/notifications/advertisements");
    } catch (error) {
    ...
    <div className="min-h-[80vh]">
      <div className="bg-white p-6 shadow">
        <Header
          ...
          onGoBack={() => navigate("/admin/notifications/advertisements")}
        />
        <div className="">
          <form onSubmit={handleSubmit} noValidate>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <DocumentUploader
                ...
                existingFileUrl={
                  existingFileNames.en_adphoto
                    ? `${
                        import.meta.env.VITE_API_BASE_URL
                      }/uploads/advertisements/${existingFileNames.en_adphoto}`
                    : null
                }
                onFileChange={(file, error) =>
                  handleFileChange(file, error, "en_adphoto")
                }
              ...
              <DocumentUploader
                ...
                existingFileUrl={
                  existingFileNames.od_adphoto
                    ? `${
                        import.meta.env.VITE_API_BASE_URL
                      }/uploads/advertisements/${existingFileNames.od_adphoto}`
                    : null
                }
                onFileChange={(file, error) =>
                  handleFileChange(file, error, "od_adphoto")
                }
            ...
            <div className="mt-6">
              <FormField
                type="url"
                label="Advertisement Link (Optional)"
                name="ad_link"
