...
const INITIAL_FORM_STATE = { en_title: "", od_title: "", date: "", expiry_date: "" };
...
const TenderFormPage = () => {
  ...
  const [files, setFiles] = useState(INITIAL_FILES_STATE); // For new file uploads
  const [existingFileNames, setExistingFileNames] = useState(INITIAL_EXISTING_FILES_STATE);
  const [filesToRemove, setFilesToRemove] = useState({ nit_doc: false, doc: false });
  ...
  useEffect(() => {
    ...
    const fetchTenderData = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/tenders/${id}`, { withCredentials: true });
        const { en_title, od_title, date, expiry_date, nit_doc, doc } = response.data;

        // CHANGED: Structure the fetched data
        const fetchedData = { en_title, od_title, date: date || "", expiry_date: expiry_date || "" };
  ...
  const handleFileChange = (fieldName, file, errorMessage) => {
    ...
    setExistingFileNames(prev => ({ ...prev, [fieldName]: '' })); // New file overrides existing one
    setFilesToRemove(prev => ({ ...prev, [fieldName]: false }));
  };

  const handleRemoveFile = useCallback((fieldName) => {
    setFiles(prev => ({ ...prev, [fieldName]: null })); // Clear any new file
    setExistingFileNames(prev => ({ ...prev, [fieldName]: '' })); // Clear existing file name
    setFilesToRemove(prev => ({ ...prev, [fieldName]: true })); // Mark for removal on submit
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    const url = isEditMode ? `${import.meta.env.VITE_API_BASE_URL}/tenders/${id}` : `${import.meta.env.VITE_API_BASE_URL}/tenders`;
    ...
    try {
      ...
      showModal("success", `Tender ${isEditMode ? 'updated' : 'added'} successfully!`);
      navigate("/admin/notifications/tenders");
    } catch (error) {
