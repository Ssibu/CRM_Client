...
const AddSubMenu = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const menusResponse = await axios.get(`${API_BASE_URL}/api/menus`, {withCredentials:true});
        setParentMenus(menusResponse.data.data || []);

        if (isEditMode) {
          const res = await axios.get(`${API_BASE_URL}/api/submenus/${id}`, {withCredentials:true});
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.titleEnglish.trim()) newErrors.titleEnglish = "Title (In English) is required";
    if (!formData.titleOdia.trim()) newErrors.titleOdia = "Title (In Odia) is required";
        if (!formData.image && !isEditMode ) newErrors.image = "Image is required";
    if (!formData.descriptionEnglish.trim()) newErrors.descriptionEnglish = "Description (In English) is required";
    if (!formData.descriptionOdia.trim()) newErrors.descriptionOdia = "Description (In Odia) is required";
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="md:col-span-2">
              ...
              {errors.menuId && <p className="mt-1 text-xs text-red-600">{errors.menuId}</p>}
            </div>
