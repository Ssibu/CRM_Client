...
const {showModal} = useModal()
  ...
  useEffect(() => {
    if (isEditMode) {
      const fetchPageData = async () => {
        try {
          const res = await axios.get(  `${process.env.REACT_APP_API_URL}/pages/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        await axios.put(  `${process.env.REACT_APP_API_URL}/pages/${id}`, formData, {
          withCredentials: true,
        });
          showModal("success", "Page updated successfully.")
      } else {
        await axios.post(  `${process.env.REACT_APP_API_URL}/pages`, formData, {
          withCredentials: true,
        });
