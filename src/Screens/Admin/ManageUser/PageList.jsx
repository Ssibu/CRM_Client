...
//         setPages(res.data);
//       } catch (error) {
//         showModal("error", "Error fetching pages:", error)
//       }
//     };
//     fetchPages();
//   }, []);
...
//     try {
//       const res = await axios.patch(`${process.env.REACT_APP_API_URL}/pages/${selectedPage.id}/status`, {}, {
//         withCredentials: true,
//       });
//       setPages((prev) =>
//         prev.map((p) => (p.id === selectedPage.id ? res.data.data : p))
//       );
//       showModal(
//         "success",
//         `Page has been ${res.data.data.isActive ? "activated" : "deactivated"} successfully`
//       );
//     } catch (error) {
...
//             </button>
//                    <button
//               onClick={() => openModal(row.original, "toggle")}
//               className={row.original.isActive ? "text-red-500" : "text-green-500"}
//                   title={row.original.isActive ? "Deactivate Page" : "Activate Page"}
//             >
//               {row.original.isActive ? <IoClose /> : <FaCheck />}
//             </button>
