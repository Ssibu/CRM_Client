  ...
  const columns = useMemo(() => [
    ...
    { header: "Title (Odia)", accessor: "od_title", isSearchable: true, isSortable: true },
    {
      ...
      cell: ({ row }) => {
        ...
        const fileUrl = `${import.meta.env.VITE_API_BASE_URL}/uploads/bed_strengths/${filename}`;         const extension = filename.split('.').pop().toLowerCase();
        const getIcon = () => {
          ...
          if (['jpg', 'jpeg', 'png'].includes(extension)) return <FaFileImage className="text-green-500" size={22} />;
