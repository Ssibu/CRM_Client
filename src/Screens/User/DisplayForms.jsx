...
const DisplayForms = () => {
  ...
  // ✅ Check if form is NEW (within 7 days)
  const isNewForm = (created_at) => {
  ...
  // ✅ Get language-based title
  const getTitle = (row) => {
    const title = language === "od" ? row.original.od_title : row.original.en_title;
    return title || "—";
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          ...
          {isNewForm(row.original.created_at) && (
            <img
              src="https://mtpl.work/dph/assets/user/images/gif-new.gif"
              alt="New"
              className="w-8"
    ...
    },
    {
      ...
      cell: ({ row }) =>
        row.original.document ? (
          <a
            href={`http://localhost:5000/uploads/forms/${row.original.document}`}
            target="_blank"
            rel="noopener noreferrer"
            className="text-red-600 text-xl"
            aria-label={`${translate("document")} for ${getTitle(row)}`}
