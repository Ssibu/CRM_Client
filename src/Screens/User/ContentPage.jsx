...
const DynamicPage = () => {
  ...
  const metaTitle = content.meta_title;
  const metaKeywords = content.meta_keyword;
  const metaDescription = content.meta_description;

  const maxLength = 8000;
  const isLong = sanitizedDescription.length > maxLength;
  const preview = isLong
    ? sanitizedDescription.slice(0, maxLength) + "..."
    : sanitizedDescription;

  return (
    <>
      {/* metas */}
      <title>{metaTitle || translate("project-title")}</title>
