...
export default function AccessibilityWindow({ isOpen, onClose }) {
  ...
  const {
    ...
    setIsBigCursor,
    isNightMode,
    setIsNightMode, // We need both the value and the setter
    resetAllSettings,
  } = useContext(AccessibilityContext);
  const { language, changeLanguage } = useLanguage();

  // Local state for the UI of this component (tabs, sliders, etc.)
  ...
  const toolOptions = [
    ...
    {
      id: "translate",
      label: language === "en" ? "Switch to Odia" : "Switch to English",
      icon: <Languages />,
      type: "toggle",
    },
    {
  ...
  const handleButtonClick = (opt) => {
    if (opt.type === "toggle") {
      if (opt.id === "nightMode") 
      {
        if(window.confirm("Are you sure you want to change theme?")){
 ...
 if (opt.id === "translate") {
  ...
  if (
    window.confirm(
      `Are you sure you want to switch to ${newLang === "en" ? "English" : "Odia"}?`
    )
  ) {
              ...
              {activeTab === "Adjustments" && (
                <>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols- gap-3">
                    {renderOptions(adjustmentOptions)}
                  </div>
                </>
              )}
              {activeTab === "Tools" && (
