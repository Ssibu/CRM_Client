...
export default function AccessibilityWindow({ isOpen, onClose }) {
  // --- STATE MANAGEMENT ---
  // Connect to the global context for style settings
  const {
    fontSize, setFontSize,
    letterSpacing, setLetterSpacing,
    lineHeight, setLineHeight,
    isBigCursor, setIsBigCursor,
    isNightMode, // We read this to know the current theme
    resetAllSettings,
  } = useContext(AccessibilityContext);

  // Get router hooks to read and manipulate the URL
  const navigate = useNavigate();
  const location = useLocation();
  const { theme } = useParams();

  // Local state for the UI of this component
  const [activeTab, setActiveTab] = useState("Adjustments");
  const [openSlider, setOpenSlider] = useState(null);
  const [isMaximized, setIsMaximized] = useState(false);
  const [isSpeakerEnabled, setIsSpeakerEnabled] = useState(false);

  // --- DATA ARRAYS FOR BUTTONS ---
  ...
  const handleButtonClick = (opt) => {
    // Handle Toggles that affect the URL
    if (opt.id === 'nightMode') {
      const newTheme = isNightMode ? 'light' : 'dark';
      // Construct the new URL path by replacing the old theme
      const newPath = location.pathname.replace(`/${theme}/`, `/${newTheme}/`);
      // Navigate to the new URL. This triggers the useEffect in NonAuthLayout.
      navigate(newPath);
      return; // Stop here for this button
    }

    // Handle Toggles that affect context directly
    if (opt.id === 'bigCursor') {
  ...
  const renderOptions = (options) =>
    options.map((opt) => {
      // For items that are just links (Search, Sitemap)
      if (opt.type === 'link') {
        return (
          <Link to={opt.path} key={opt.id} className="block">
            <motion.div
              whileHover={{ y: -2 }}
              whileTap={{ scale: 0.98 }}
              className="flex flex-col items-center justify-center p-4 rounded-lg bg-gray-50 hover:bg-blue-50 border border-gray-200 hover:border-blue-300 transition-all duration-200 h-full"
            >
      ...
      // For all other interactive buttons
      return (
          ...
          >
            ...
            <div className="flex justify-between items-center px-5 py-4 text-white bg-gradient-to-r from-indigo-600 to-amber-300">
              <div>
                ...
                <p className="text-sm opacity-90">Customize your browsing experience</p>
              </div>
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-2 rounded-full hover:bg-white/20 text-white focus:outline-none focus:ring-2 focus:ring-white transition-colors"
                >
                  <X />
                </button>
              </div>
