...
const isNew = (dateString) => {
  ...
  const diffDays = diffTime / (1000 * 60 * 60 * 24);
  return diffDays >= 0 && diffDays <= 7; // Check if the item is within the last 7 days
};

// --- Reusable Components ---

// Renders a single list item within a card
const BoardItem = ({ item, apiBaseUrl }) => {
  const itemTitle = (
    <>
      {item.title}
      {isNew(item.originalDate) && (
        <img
          src="https://mtpl.work/dph/assets/user/images/gif-new.gif"
          alt="New"
          className="w-auto h-3 ml-2" // Added margin for spacing
        />
      )}
    </>
  );
    ...
    <div className="flex items-center gap-4">
      ...
      <div className="flex-1">
        {item.doc ? (
          <a
            title={item.title}
            href={`${apiBaseUrl}/uploads/${item.folder}/${item.doc}`} // This provides the link to the item
            target="_blank"
            rel="noopener noreferrer"
            className="text-[#2563eb] items-center hover:text-green-600 font-medium flex"
          >
            {itemTitle}
          </a>
        ) : (
          <p className="text-[#2563eb] font-medium flex items-center">{itemTitle}</p>
        )}
      </div>
    </div>
  );
...
// Renders an entire card, including its title, list of items, and "Read More" link
const BoardCard = ({ card, index, apiBaseUrl, translate }) => (
  <motion.article
    className="relative bg-[#f1f1f1f0] h-[440px] rounded-xl border-b-[5px] border-[#ffc107] shadow-[-1px_-10px_8px_#56565654] p-6 flex flex-col transition-transform duration-300 hover:scale-[1.03] hover:shadow-lg focus-within:ring-2 focus-within:ring-yellow-500"
    variants={{
      hidden: { opacity: 0, y: 30 },
    ...
    <div className="mt-16 flex-grow overflow-y-auto space-y-4 pr-2">
      {card.items.length > 0 ? (
        card.items.map((item, idx) => (
          <BoardItem key={idx} item={item} apiBaseUrl={apiBaseUrl} />
        ))
      ) : (
        <p className="text-center pt-16 text-gray-500">
          {translate("No")} {card.title?.toLowerCase()} {translate("found")}.
        </p>
      )}
    </div>

    <Link
      to={`/subpage/${card.slug}`}
      className="absolute bottom-[-1.5rem] left-[9.5rem] bg-yellow-500 text-black px-8 py-2 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-600 transition-colors duration-300 hover:bg-yellow-400"
...
export default function Board() {
  ...
  const cardConfigurations = useMemo(() => [
    { key: 'tender', titleKey: 'tender', slug: 'tenders', folder: 'tenders', dateField: 'date', docField: 'doc' },
    { key: 'notice', titleKey: 'notice', slug: 'notice-advertisements', folder: 'notices', dateField: 'date', docField: 'doc' },
    { key: 'actAndRule', titleKey: 'acts-and-rules', slug: 'acts-rules', dateField: 'createdAt' },
    { key: 'newsAndEvent', titleKey: 'news-and-events', slug: 'news-events', folder: 'events', dateField: 'eventDate', docField: 'document' },
  ], []);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const formattedData = cardConfigurations.map(config => {
          ...
          return {
            ...
            items: items.map(item => ({
              ...
              doc: config.docField ? item[config.docField] : null,
              folder: config.folder || null,
              originalDate: item[config.dateField], // **FIX**: Pass the original date for the 'isNew' check
              ...formatDate(item[config.dateField]),
            })),
          };
        });
        ...
        console.error("Error fetching latest data:", error);
        // Create empty cards on error to prevent UI crash
        setCardData(cardConfigurations.map(c => ({ 
          id: c.key, 
    ...
    fetchData();
  }, [API_URL, translate, cardConfigurations]);
