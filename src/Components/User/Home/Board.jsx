                ...
                {card.items.length > 0 ? (
                  card.items.map((item, index) => (
                    <div key={index} className="flex items-center gap-4">
                      ...
                      <div className="flex-1">
                        {item.doc ? (
                          <a title={item.title} href={`${API_URL}/uploads/${item.folder}/${item.doc}`} target="_blank" rel="noopener noreferrer" className="text-[#2563eb] hover:text-green-600 font-medium">
                            {item.title}
                          </a>
                        ) : (
                          <p className="text-[#2563eb] font-medium">{item.title}</p>
